...
class AcpecFuelTransaction(models.Model):
    ...
    name = fields.Char(
        ...
        copy=False,
        index=True,
        help="Référence technique Backoffice générée par ir.sequence. Ne pas l'exposer comme référence publique mobile.",
    )
    operation_ref = fields.Char(
        ...
        copy=False,
        index=True,
        help="Référence publique/mobile non énumérable au format TX-YYYYMMDD-HHMMSS-NNNNNNNNNNNN. Plusieurs lignes append-only peuvent partager la même opération.",
    )
    ...
    @api.model
    def _generate_unique_transaction_reference(self, reserved_refs=False):
        reserved_refs = reserved_refs or set()
        for _attempt in range(self.TX_REFERENCE_MAX_RETRIES):
            operation_ref = self._generate_transaction_reference_candidate()
            if operation_ref in reserved_refs:
                continue
            if not self.sudo().search_count([('operation_ref', '=', operation_ref)]):
                return operation_ref
        raise UserError('Impossible de générer une référence opération unique.')
    ...
    def log(
        ...
        tx_model = self.sudo().with_context(allow_fuel_transaction_create=True)
        if operation_ref:
            tx_model = tx_model.with_context(allow_fuel_transaction_operation_ref_override=True)
        tx = tx_model.create(vals)
        line_model = self.env['acpec.fuel.transaction.line'].sudo().with_context(
            allow_fuel_transaction_line_create=True,
        )
        for line in lines or []:
            line_model.create(dict(line, transaction_id=tx.id))
        return tx
    ...
    def create(self, vals_list):
        if not self.env.context.get('allow_fuel_transaction_create'):
            raise UserError(_(
                'La création de transactions est réservée aux flux métier internes contrôlés.'
            ))
        reserved_operation_refs = set()
        for vals in vals_list:
            # name is now a technical Backoffice reference generated by ir.sequence.
            # The public/mobile reference is operation_ref and keeps the historical
            # TX-YYYYMMDD-HHMMSS-NNNNNNNNNNNN generator.
            if (
                vals.get('name') in (False, None, '', 'New')
                or not self.env.context.get('allow_fuel_transaction_name_override')
            ):
                vals['name'] = self._generate_transaction_sequence_name()

            if (
                vals.get('operation_ref') in (False, None, '', 'New')
                or not self.env.context.get('allow_fuel_transaction_operation_ref_override')
            ):
    ...
    def write(self, vals):
        ...
        if vals_keys & regularization_fields and not (
            self.env.context.get('allow_fuel_transaction_regularization_update')
            or self.env.context.get('allow_fuel_transaction_update')
        ):
            raise UserError(_('La régularisation station doit passer par l’action dédiée.'))

        protected_keys = vals_keys - regularization_fields
        if protected_keys and not self.env.context.get('allow_fuel_transaction_update'):
            raise UserError(_(
                'Les transactions sont append-only hors flux interne contrôlé.'
            ))
    ...
    def unlink(self):
        if not self.env.context.get('allow_fuel_transaction_unlink'):
            raise UserError(_(
                'Les transactions ne peuvent pas être supprimées hors flux interne contrôlé.'
            ))
        return super().unlink()

    def init(self):
        # Patch43M20-A: preserve historical public TX references when adding
        # operation_ref. Existing rows keep their current name, while new rows
        # use name as a technical Backoffice sequence.
        self.env.cr.execute(
            """
            UPDATE acpec_fuel_transaction
               SET operation_ref = name
             WHERE (operation_ref IS NULL OR operation_ref = '')
...
class AcpecFuelTransactionLine(models.Model):
    ...
    @api.model_create_multi
    def create(self, vals_list):
        if not self.env.context.get('allow_fuel_transaction_line_create'):
            raise UserError(_(
                'La création de lignes transactionnelles est réservée aux flux métier internes contrôlés.'
            ))
        return super().create(vals_list)

    def write(self, vals):
        if vals and not (
            self.env.context.get('allow_fuel_transaction_line_update')
            or self.env.context.get('allow_fuel_transaction_update')
        ):
            raise UserError(_(
                'Les lignes transactionnelles sont append-only hors flux interne contrôlé.'
            ))
        return super().write(vals)
    ...
    def unlink(self):
        ...
        if not (
            ...
            raise UserError(_(
                'Les lignes transactionnelles ne peuvent pas être supprimées hors flux interne contrôlé.'
            ))
        return super().unlink()
