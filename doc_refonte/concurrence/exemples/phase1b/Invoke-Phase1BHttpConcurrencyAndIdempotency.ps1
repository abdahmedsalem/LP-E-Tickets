...
# - three controlled concurrency collisions;
# - same-key replay after a committed result;
...
$OutputDirectoryName = "phase1b_http_$Stamp"
...
manifest = {
    ...
    "replay_case": single_case(
        "replay",
        "phase1b-replay-same-key",
    ),
    "mismatch_case": single_case(
        "mismatch",
        "phase1b-mismatch-same-key",
    ),
}
...
function Build-DatabaseEvidenceSql {
    ...
    foreach ($Case in $Manifest.concurrency_cases) {
        $FaceLineIds += [int]$Case.face_line_id
        $QrIds += [int]$Case.request_one.qr_id
        $QrIds += [int]$Case.request_two.qr_id
        $Keys += [string]$Case.request_one.idempotency_key
        $Keys += [string]$Case.request_two.idempotency_key
    }

    $FaceLineIds += [int]$Manifest.replay_case.face_line_id
    $FaceLineIds += [int]$Manifest.mismatch_case.face_line_id
    $QrIds += [int]$Manifest.replay_case.qr_id
    $QrIds += [int]$Manifest.mismatch_case.qr_id
    $Keys += [string]$Manifest.replay_case.idempotency_key
    $Keys += [string]$Manifest.mismatch_case.idempotency_key

    $FaceLineIdList = (($FaceLineIds | Sort-Object -Unique) -join ",")
    $QrIdList = (($QrIds | Sort-Object -Unique) -join ",")
    $EscapedKeys = @()

    foreach ($Key in ($Keys | Sort-Object -Unique)) {
        $EscapedKeys += "'" + $Key.Replace("'", "''") + "'"
    }

    $KeyList = $EscapedKeys -join ","
...
try {
    Add-RuntimeLine -Text "FuelToken Phase 1B-A HTTP diagnostic"
    Add-RuntimeLine -Text "harness_version=1B-A-v3-dbfilter-guard"
...
"@
    ...
    $Replay = $Manifest.replay_case
    $ReplayParameters = @{
        public_code = [string]$Replay.public_code
        action_code = [string]$Manifest.pin
        idempotency_key = [string]$Replay.idempotency_key
    }

    $RequestCounter++
    $ReplayBodyOne = New-JsonRpcBody -Parameters $ReplayParameters -RequestId $RequestCounter
    Invoke-JsonRpcRequest -Label "replay_first_committed_response_discarded" -Token ([string]$Replay.station.access_token) -Body $ReplayBodyOne -TargetPath (Join-Path $HttpDirectory "replay_first.json") | Out-Null

    $RequestCounter++
    $ReplayBodyTwo = New-JsonRpcBody -Parameters $ReplayParameters -RequestId $RequestCounter
    Invoke-JsonRpcRequest -Label "replay_second_same_key_same_payload" -Token ([string]$Replay.station.access_token) -Body $ReplayBodyTwo -TargetPath (Join-Path $HttpDirectory "replay_second.json") | Out-Null

    $Mismatch = $Manifest.mismatch_case
    $MismatchParametersOne = @{
        public_code = [string]$Mismatch.public_code
        action_code = [string]$Manifest.pin
        idempotency_key = [string]$Mismatch.idempotency_key
        pump_no = "A"
    }

    $RequestCounter++
    $MismatchBodyOne = New-JsonRpcBody -Parameters $MismatchParametersOne -RequestId $RequestCounter
    Invoke-JsonRpcRequest -Label "mismatch_first_payload_A" -Token ([string]$Mismatch.station.access_token) -Body $MismatchBodyOne -TargetPath (Join-Path $HttpDirectory "mismatch_first.json") | Out-Null

    $MismatchParametersTwo = @{
        public_code = [string]$Mismatch.public_code
        action_code = [string]$Manifest.pin
        idempotency_key = [string]$Mismatch.idempotency_key
        pump_no = "B"
    ...
    $RequestCounter++
    $MismatchBodyTwo = New-JsonRpcBody -Parameters $MismatchParametersTwo -RequestId $RequestCounter
    Invoke-JsonRpcRequest -Label "mismatch_second_payload_B" -Token ([string]$Mismatch.station.access_token) -Body $MismatchBodyTwo -TargetPath (Join-Path $HttpDirectory "mismatch_second.json") | Out-Null

    $CaseIndex = 0

    foreach ($Case in $Manifest.concurrency_cases) {
        $CaseIndex++
        $CaseLabel = "concurrency_case_$CaseIndex"
        ...
        $JobOne = Start-HttpJob -Label ($CaseLabel + "_request_a") -Token ([string]$Case.request_one.station.access_token) -Body $BodyOne
        $JobTwo = Start-HttpJob -Label ($CaseLabel + "_request_b") -Token ([string]$Case.request_two.station.access_token) -Body $BodyTwo

        Save-HttpJobResult -Job $JobOne -TargetPath (Join-Path $HttpDirectory ($CaseLabel + "_request_a.json")) | Out-Null
        Save-HttpJobResult -Job $JobTwo -TargetPath (Join-Path $HttpDirectory ($CaseLabel + "_request_b.json")) | Out-Null
    ...
    if ($HttpResultCount -ne 10) {
        $script:MainCode = 41
        Add-RuntimeLine -Text "warning=expected_10_http_results_got_$HttpResultCount"
    }
}
catch {
    ...
    Add-ReportLine -Path $SummaryPath -Text "FuelToken Phase 1B-A HTTP summary"
...
Write-Host "Phase 1B-A HTTP diagnostic complete."
