# Spec d’architecture — Refonte FuelToken v3.3

## 0. Statut du document

Document de cadrage figé avant les patches de code.

Version v3.3 : ajout de la doctrine de séparation mobile / société, limitation stricte des changements backend existants et doctrine back-office **no create / no delete** sur les vues opérationnelles.
...
- la majorité des vues back-office opérationnelles sont en **no create / no delete**.
...
Règle générale figée : limiter au maximum les changements backend existants. Le chantier société doit créer une nouvelle surface web/portal séparée, sans transformer les APIs mobile existantes.

Découpage de référence :

```text
Mobile users       = flux mobile existant
Comptes Sociétés   = flux portal/web séparé
...
Ces gardes doivent rester invisibles pour le flux mobile normal et ne doivent pas introduire un contrat API société dans l’API mobile.

### 2.3 Règle spéciale téléphone / inscription mobile

On ne modifie pas l’API mobile pour gérer spécialement les sociétés.

Si un utilisateur mobile essaie de s’inscrire avec un téléphone déjà reconnu par le flux mobile existant, et que l’API existante répond naturellement :
...
On ne rajoute pas dans l’API mobile une logique spéciale du type :

```text
ce téléphone appartient à une société distributrice
```

Le téléphone de contact d’un Compte Société est une donnée de contact web/back-office. Il ne doit pas forcer une modification du comportement mobile existant.

### 2.4 Avocat du diable
...
10. Un Compte Société peut avoir un téléphone de contact web/back-office, mais ce téléphone ne transforme pas la société en utilisateur mobile et ne modifie pas le signup mobile.
11. Un employé est un utilisateur mobile normal.
12. Un employé peut être rattaché à une seule société.
13. Après réception d’un carnet, l’employé utilise son wallet indépendamment de la société.
14. La société ne suit pas le wallet privé de l’employé après distribution.
...
- le téléphone mobile reste unique dans le périmètre utilisé par les flux mobile existants ;
- le Compte Société peut avoir un téléphone de contact séparé ;
- ce téléphone de contact société ne doit pas obliger à modifier les APIs mobile ;
- on ne rajoute pas de message ou de branche spéciale dans le signup mobile pour dire qu’un téléphone appartient à une société distributrice ;
- si le comportement mobile existant répond naturellement “ce user existe déjà”, on conserve ce comportement.
...
| Transfer | Historique des transferts de carnets / Distribution |
...
### 10.4 Doctrine back-office : no create / no delete

Règle générale : la majorité des vues back-office opérationnelles et d’audit doivent être en **no create / no delete**.

Objectif : éviter qu’un opérateur crée ou supprime manuellement des objets métier qui doivent normalement provenir d’un workflow contrôlé, d’une API, d’une validation ou d’une action serveur.

Doivent être no create / no delete par défaut :

- wallets / comptes clients ;
- crédits disponibles / face lines ;
- QR / bons de retrait ;
- transactions et lignes de transaction ;
- transferts / distributions historiques ;
- sessions mobile ;
- OTP / vérifications ;
- demandes de comptes mobiles, sauf actions explicites de validation/rejet ;
- historiques et journaux opérationnels.

La vue peut rester éditable pour les champs fonctionnels nécessaires, mais la création et la suppression directes doivent être désactivées.

Exceptions autorisées :

- configuration : formules de carnet, politiques, paramètres ;
- stations, si création back-office retenue ;
- Comptes Sociétés, car ils sont explicitement créés par le back-office ;
- données de paramétrage ou d’onboarding créées volontairement par ACPEC.

Même dans les exceptions, préférer l’archivage ou la désactivation à la suppression physique lorsqu’il existe un impact d’audit.
...
- application de la doctrine no create / no delete sur les vues opérationnelles lorsque c’est possible sans changer le métier.
...
- transferts ;
- stations ;
- désactivation systématique de create/delete dans les vues d’audit et d’opérations, sauf exceptions documentées.
...
- vues Compte Société ;
- smart buttons ;
- aucun changement du signup mobile ni des APIs mobile existantes.

### Patch 4 — Gardes serveur société

Objectif : sécurité stricte sans refonte du mobile.
...
- ne pas ajouter de logique spéciale société dans les routes de signup mobile, sauf faille réelle démontrée.
...
15. Backend existant touché au minimum, surtout les APIs mobile.
16. Pas de logique spéciale “téléphone société” dans l’API mobile.
17. Téléphone mobile unique dans le périmètre mobile existant ; téléphone de contact société séparé.
18. Vues back-office opérationnelles majoritairement en no create / no delete.
...
7. durcir le backend mobile pour traiter un cas société qui appartient au nouveau portail web ;
8. laisser les opérateurs créer ou supprimer manuellement des objets opérationnels qui doivent venir d’un workflow contrôlé.
