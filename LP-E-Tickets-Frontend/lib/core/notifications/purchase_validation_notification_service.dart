...
class PurchaseValidationNotificationService {
  ...
  Future<void> syncForUser(AppUser user) async {
    ...
    try {
      ...
      final prefsInitialized = await _hasNotifiedIds(
        user.id,
        prefix: _prefsPrefix,
      );
      final notifiedIds = await _loadNotifiedIds(user.id);
      await _syncPurchaseNotifications(
        user,
        notifiedIds,
        emitNew: prefsInitialized,
      );
      await _syncTransferNotifications(
        user,
        notifiedIds,
        emitNew: prefsInitialized,
      );
      ...
      final qrPrefsInitialized = await _hasNotifiedIds(
        user.id,
      ...
      await _syncQrExpirationNotifications(
        user,
        qrNotifiedIds,
        emitNew: qrPrefsInitialized,
      );
  ...
  Future<void> _syncTransferNotifications(
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint(
          '[purchase-validation] sync des notifications de transferts ignorée: ${e.runtimeType}',
        );
      }
    }
  }
  ...
  Future<void> _syncQrExpirationNotifications(
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint(
          '[purchase-validation] sync des notifications d\'expiration QR ignorée: ${e.runtimeType}',
        );
      }
    }
  }
  ...
  Future<NotificationItem> _buildStationConsumptionNotification(
    ...
    final body = _stationConsumptionBody(amountLabel, station, dateLabel);
  ...
  String _stationConsumptionBody(
    String amountLabel,
    String station,
    String dateLabel,
  ) {
    ...
    if (amountLabel.isEmpty) {
      return 'Utilisation confirmée $place le $dateLabel';
    }
    return '$amountLabel utilisés $place le $dateLabel';
  }
