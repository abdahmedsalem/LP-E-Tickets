...
void main() {
  group('session timeout recovery guard', () {
    ...
    test(
      ...
      () {
        ...
        expect(
          source,
          contains(
            'e.requiresLogout || e.isAuthRequired || e.isOdooSessionExpired',
          ),
        );
        expect(
          source,
    ...
    test('refresh recovery uses stale-token check and single-flight latch', () {
      ...
      expect(
        source,
        contains('Future<_SilentRefreshResult>? _refreshInFlight'),
      );
      expect(
        source,
        contains('late final Future<_SilentRefreshResult> tracked'),
      );
    ...
    test(
      ...
      () {
        ...
        expect(
          source,
          contains('loadError: ErrorPresenter.backendUnavailable()'),
        );
        expect(
          source,
          contains('emit(state.copyWith(loading: true, clearError: true));'),
        );
        expect(
          source,
          isNot(
            contains('loading: true, clearError: true, clearBreakdown: true'),
          ),
        );
    ...
    test('client home shows backend unavailable banner with retry', () {
      final source = File(
      ...
      expect(source, contains('AppLocalizations.of(context).commonRetry'));
      expect(source, contains('ctx.read<WalletCubit>().refresh()'));
    });

    test('error presenter exposes backend unavailable classification', () {
      final source = File(
