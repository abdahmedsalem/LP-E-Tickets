...
class _SplitQrScreenState extends State<SplitQrScreen> {
  ...
  void initState() {
    ...
    if (AppEnvironment.useAcpecLiveData) {
      ...
      _loadError = 'Connexion serveur ACPEC requise pour éclater un QR.';
    }
  }
  ...
  void _applyParent(QrToken? parent) {
    ...
    if (!AcpecQrSplitBuilder.canSplitParent(parent)) {
      setState(() {
        _parent = parent;
        _loadError =
            'Ce QR ne peut plus être éclaté (état : ${parent.state.label}).';
      });
      return;
    }
  ...
  Future<void> _submit() async {
    ...
    try {
      if (AppEnvironment.useAcpecLiveData) {
        ...
        throw Exception('Connexion serveur ACPEC requise pour éclater un QR.');
      }
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('QR éclaté avec succès.')));
        context.pop();
      }
    ...
    if (AppEnvironment.useAcpecLiveData && _loadingParent) {
      return Scaffold(
        body: SafeArea(
          child: Column(
            children: [
              AppBarHeader(title: 'Éclater le QR', onBack: () => context.pop()),
              Expanded(
                child: ListView(
...
class _BlockedSplitHint extends StatelessWidget {
  const _BlockedSplitHint({required this.valid, required this.expired});
...
class _PartitionCard extends StatelessWidget {
  ...
  int _ticketsInPartition() => picks.values.fold(0, (s, v) => s + v);

  int _totalAmountInPartition() {
    var total = 0;
    for (final line in sortedLines) {
      final qty = picks[line.id] ?? 0;
      if (qty > 0) total += qty * line.faceValue;
    }
    return total;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final totalAmount = _totalAmountInPartition();
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      if (inPart > 0)
                        Text(
                          'Total: ${Formatters.numberFr(totalAmount)} MRU',
                          style: const TextStyle(
                            fontSize: 12,
                            fontWeight: FontWeight.w800,
                            color: AppColors.ink,
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          _SplitStepper(value: value, max: max, onChange: onChange),
        ],
      ),
    );
  }
