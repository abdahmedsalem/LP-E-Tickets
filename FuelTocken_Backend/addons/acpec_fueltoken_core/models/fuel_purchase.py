    ...
    def _create_face_lines_after_approval(self):
        ...
        for purchase in self:
            ...
            with self.env.cr.savepoint():
                wallet = self.env['acpec.fuel.wallet'].sudo().get_or_create(purchase.partner_id, purchase.company_id)
                tx_lines = []
                for line in purchase.line_ids:
                    expires_at = False
                    if line.carnet_type_id.validity_days:
                        expires_at = fields.Datetime.add(purchase.approved_at or fields.Datetime.now(), days=line.carnet_type_id.validity_days)
                ...
                tx_model.log('achat_carnets', purchase.company_id, wallet=wallet, purchase=purchase, lines=tx_lines, idempotency_key=purchase.idempotency_key)
