                  ...
                  Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        const SizedBox(width: 8),
                        Text(
                          '${Formatters.numberFr(totalTickets!)} tickets actifs',
...
class _WalletBreakdownParsers {
  ...
  static List<Map<String, dynamic>>? faceRows(dynamic raw) {
    if (raw is List) {
      ...
      out.sort(
        (a, b) => _n(b['face_value'] ?? b['denomination'] ?? b['value'])
  ...
  static List<Map<String, dynamic>>? carnetRows(dynamic raw) {
    if (raw is List) {
      ...
      for (final e in raw) {
        if (e is! Map) continue;
        final m = Map<String, dynamic>.from(e);
        final hasCarnet = m.containsKey('carnet_type_id') ||
        ...
        children: [
          ...
          Column(
            ...
            children: [
              ...
              Text(
                '${Formatters.numberFr(entry.amountMru)} MRU',
                style: const TextStyle(
                  fontSize: 11,
                  fontWeight: FontWeight.w600,
...
class _FaceExpiryTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final exp = m['expiration_date']?.toString() ??
        m['expiry']?.toString() ??
        m['expires_on']?.toString();
    final lot = m['lot_ref']?.toString() ??
        m['purchase_ref']?.toString() ??
        m['lot_id']?.toString();

    if (fv <= 0 && exp == null && lot == null) {
      return _GenericKeyValueCard(
        entries: m.entries.toList(),
      );
    }
