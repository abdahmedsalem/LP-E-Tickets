    ...
    if (AppEnvironment.useAcpecLiveData) {
      ...
      try {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Lot rejeté. Le motif a été enregistré.',
            ),
          ),
        );
      } on OdooJsonRpcException catch (e) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          ...
          children: [
            AppBarHeader(
              title: _confirmingReject
                  ? 'Rejeter l’achat'
                  : 'Détail de l’achat',
              subtitle: _lot != null
                  ? (_confirmingReject
                        ? (_hideTechnicalRefs
                              ? 'Lot à valider · motif obligatoire'
                              : '${_lot!.internalRef} · motif obligatoire')
                        : (widget.adminMode
                              ? Formatters.money(_lot!.totalAmount)
                              : (_lot!.publicCode.trim().isNotEmpty
                                    ? _lot!.publicCode
                                    : _lot!.internalRef)))
...
class _ProofTile extends StatelessWidget {
  ...
  IconData _iconForMime() {
    final m = proof.mimeType?.toLowerCase() ?? '';
  ...
  bool get _showImagePreview =>
      (proof.hasImagePreview &&
          (isImageMimeType(proof.mimeType, filename: proof.filename) ||
              looksLikeImageFilename(proof.filename))) ||
      (proof.url != null &&
          proof.url!.isNotEmpty &&
          (isImageMimeType(proof.mimeType, filename: proof.filename) ||
              looksLikeImageFilename(proof.filename)));
  ...
  Widget build(BuildContext context) {
    final url = proof.url?.trim();
    final name = proof.filename?.trim();
    final date = proof.uploadedAt;
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (date != null) ...[
                        const SizedBox(height: 6),
                        Text(
                          'Ajouté le ${Formatters.dateTimeDash(date)}',
                          style: const TextStyle(
                            fontSize: 11,
                            color: AppColors.muted,
                          ),
