...
class AcpecQrMapper {
  ...
  static Map<String, dynamic> _unwrap(dynamic result) {
    ...
    if (err is Map && m['ok'] != true) {
      throw Exception(
        err['message']?.toString() ?? 'Opération QR refusée.',
      );
    }
    final data = m['data'];
    if (data is Map) {
  ...
  static List<QrLine> _lines(
    ...
    for (var i = 0; i < v.length; i++) {
      ...
      var fv = _int(row['face_value'] ?? row['nominal'] ?? row['unit_value'], 0);
      final qty = _int(row['qty'] ?? row['quantity'], 0);
      if (qty <= 0) continue;
      if (fv <= 0) {
        final amt = _int(
          row['amount'] ?? row['line_amount'] ?? row['subtotal'] ?? row['total'],
          0,
        );
        if (amt > 0) {
          fv = (amt / qty).round();
      ...
      out.add(
        QrLine(
          ...
          lotInternalRef: row['lot_ref']?.toString() ??
              ...
              '—',
          faceLineId:
              row['face_line_id']?.toString() ?? row['acpec_line_id']?.toString() ?? 'fl-$i',
          faceValue: fv,
          qty: qty,
          expirationDate: exp,
        ),
  ...
  static QrToken qrFromDataMap(
    ...
    final publicCode = row['public_code']?.toString().trim() ??
    ...
    return QrToken(
      ...
      companyId: companyId,
      state: state,
      parentQrId: row['parent_id']?.toString() ?? row['parent_qr_id']?.toString(),
      lines: lines,
      createdAt: created,
  ...
  static QrToken fromStationUseResult(
    ...
    final resolvedCode = m['public_code']?.toString().trim() ??
