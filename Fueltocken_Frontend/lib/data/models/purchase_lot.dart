...
class PurchaseLine extends Equatable {
  ...
  final String carnetTypeCode; // C10-500
  final int carnetCount; // number of tickets (booking unit)
  final int carnetSize; // faces per ticket
  final int faceValue;
...
class PurchaseProofSummary extends Equatable {
  ...
  /// Contenu binaire (souvent image) décodé depuis `proof_data` API.
  ...
  /// Preuve exploitable à l’écran (binaire, URL ou nom de fichier réel).
...
class PurchaseLot extends Equatable {
  final String id;
  final String internalRef; // ACH/2026/00001
  ...
  int get totalAmount =>
      lines.fold(0, (sum, line) => sum + line.lineAmount);
